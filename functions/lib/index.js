 ...
 */
exports.acceptInvite = functions.https.onCall(async (req) => {
    const uid = req.auth?.uid;
    if (!uid) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    const { deckId, tokenHash } = req.data || {};
    if (!deckId || !tokenHash) {
        throw new functions.https.HttpsError('invalid-argument', 'deckId and tokenHash are required');
    }
    // TODO: implement lookup of invite by tokenHash and deckId, validate, and update membership
    return { ok: true };
});
