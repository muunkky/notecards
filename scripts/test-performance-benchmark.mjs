...
- ⚠️ Warning: 50-100ms per test
- ❌ Needs optimization: > 100ms per test
...
`
  ...
  if (metrics.averageTestTime > 100) {
    console.log('⚠️  Average test time is high - consider optimizations')
  } else {
    console.log('✅ Test performance looks good!')
  }
}
